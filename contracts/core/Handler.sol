...
contract Handler is Ownable {
    ...
    function setHandler(address _handler, bool _state) external virtual onlyOwner{
        _setHandler(_handler, _state);
    }
    function allHandlers() external view returns (address[] memory) {
        return handler.valuesAt(0, handler.length());
    }

    function _setHandler(address _handler, bool _state) internal {
        if (_state){
    ...
    function setManager(address _manager, bool _state) external virtual onlyOwner{
        _setManager(_manager, _state);

    }
    function _setManager(address _manager, bool _state) internal {
        if (_state){
